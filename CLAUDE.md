...
- Tests live in `tests/` (`unit/`, `property/` with `fast-check`, `helpers/`, `fixtures/`);
  `tests/setup.ts` is preloaded and coverage thresholds are 80% line/function (`bunfig.toml`).
