...
const watchStreakArbitrary: fc.Arbitrary<WatchStreak> = fc
	...
	.map((data) => {
		const startDate = `${data.year}-${String(data.startMonth).padStart(2, '0')}-${String(data.startDay).padStart(2, '0')}`;
		const endDate = `${data.year}-${String(data.endMonth).padStart(2, '0')}-${String(data.endDay).padStart(2, '0')}`;
		return {
