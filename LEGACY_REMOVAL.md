...
> | US-021 | ✅ Closed at natural stopping point — all CTA-shaped buttons across the 4 reskin-target admin routes are on shadcn primitives. **DONE per visual-coupling pattern**: /admin/users (toggle-button × 2), /admin/logs (7 buttons), /admin/slides (12 buttons), /admin/sync (8 CTA buttons: cancel/sync/control quartet/cron-update/pagination quartet). Remaining native: preset-chip × N visual chip selectors + numeric `.pagination-page` selectors in /admin/sync — both fall under the visual-selector classifier (intentionally hand-styled UI tiles, same as substep-dot + theme-swatch in onboarding). /admin/wrapped is cinematic by design — no primary CTAs. |
