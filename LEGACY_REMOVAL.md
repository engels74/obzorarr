...
> | US-021 | ⚠️  Partial — pattern proven on 2 routes: /admin/users toggle-button Yes/No permission CTA (desktop + mobile copies) + /admin/logs clear-filters-button "Clear All". Remaining: ~30 buttons across sync (15) / logs (6) / slides (12) / wrapped (cinematic, no swap candidates). Each follows the same `:global` hoist pattern from US-024 / US-026 — mechanical, atomic per button-type. |
